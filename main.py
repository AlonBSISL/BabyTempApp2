...
# This function saves the new baby into the system and makes sure warnings are issued if necessary.
def save_baby():
...
# This allows the user to add a new baby and its basic information and gives the user the baby's ID.
...
# This is a list of babies and their information which allows the user to scroll through the babies and, when clicking on one, add new measurements and see statistics.
def open_baby_list():
...
def save_temp():
    global current_baby, add_temp, babies, baby_list_window, baby_list, counter, c
...
def add_temp_check(x):
    global current_temp_box_add, add_temp_check_window, baby_id_check, list_id, current_baby, add_temp, c
    print(x)
    print(type(x))
    list_id = x[0]
    current_baby = list(x)
